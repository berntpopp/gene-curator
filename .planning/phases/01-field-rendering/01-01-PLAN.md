...
3. **Add computed for custom component lookup with emit on fallback:**
   ```javascript
...
6. **Update template with explicit prop mapping for custom components:**
   - Add custom component rendering at TOP of template (before other v-if chains).
   - IMPORTANT: Do NOT use `v-bind="fieldSchema"` as specialized components expect specific props.
   - Map props explicitly based on what each component accepts:
     ```vue
   ...
   - The specialized components accept these common props:
     - `modelValue` (all) - the v-model value
     - `label` (all) - field label (from fieldSchema.title)
     - `hint` (all except HPOInput uses hint differently) - help text (from fieldSchema.description)
     - `required` (all) - required state
     - `disabled` (most) - disabled state

   - Component-specific props NOT passed (they use defaults):
     - MONDOAutocomplete: diseaseName, clearable, prependIcon, minSearchLength, maxResults
     - PMIDInput: publications, validateOnMount
     - HPOInheritanceSelect: hpoId, clearable, prependIcon
     - OMIMAutocomplete: multiple, clearable, prependIcon, minSearchLength, maxResults
     - HPOInput: validateOnMount

7. **Update object fields section to:**
...
8. **Update array fields section similarly with depth prop and styling**

9. **Add MAX_DEPTH warning after array/object sections:**
   ```vue
...
11. **Add 'component-fallback' to emit array in defineEmits**
  </action>
  <verify>
    ...
    - `grep -c "component-fallback" frontend/src/components/dynamic/DynamicField.vue` returns at least 2 (emit definition + emit call)
    - Run: `cd /home/bernt-popp/development/gene-curator/frontend && npm run lint -- --max-warnings 0` passes
  </verify>
  <done>DynamicField renders custom components from registry with explicit prop mapping, tracks depth, applies nested styling, emits component-fallback when component not found, and uses editable fallback for unknown types</done>
</task>
...
2. **DynamicField Enhancement:**
   ...
   - Has customComponent computed with logging AND emit('component-fallback')
   - Has getNestedStyle function
   - Template renders custom components first with explicit prop mapping (:label, :hint, :required, :disabled)
...
- Custom components receive explicit props (:label, :hint, :required, :disabled) not v-bind="fieldSchema"
- component-fallback event emitted when component specified but not in registry
